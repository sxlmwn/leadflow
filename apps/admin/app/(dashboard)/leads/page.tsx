    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 bg-white dark:bg-slate-900">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <input
            ...
            className="w-24 h-9 px-3 text-xs rounded-full bg-slate-100 dark:bg-slate-800 border-none text-slate-800 dark:text-slate-200 placeholder-slate-400"
          />
          ...
          <label className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300 cursor-pointer ml-auto">
            <input
              type="checkbox"
              checked={dncOnly}
      ...
      <Card className="p-6">
        <CardHeader className="p-0 pb-4 flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-lg font-extrabold text-slate-900 dark:text-white">Leads Master Table</CardTitle>
            <CardDescription className="text-xs font-medium text-slate-400">Showing {totalCount} total records</CardDescription>
          </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-slate-100 dark:border-slate-800">
                <TableHead className="text-xs font-bold text-slate-400">Created</TableHead>
                ...
                <TableHead className="text-xs font-bold text-slate-400">DNC Flag</TableHead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" className="text-slate-600 dark:text-slate-400 hover:text-slate-900 text-xs font-semibold">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-4xl max-h-[90vh] rounded-3xl p-6 overflow-y-auto shadow-2xl relative animate-in zoom-in-95 duration-150">
            ...
            {loadingDetails ? (
              ...
              <div className="space-y-6 text-xs">
                {/* 1. Contact & Core Info */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/60 dark:border-slate-800">
                  <div>
                    <span className="text-slate-400 font-medium">Brand Domain</span>
                    <div className="font-extrabold text-slate-900 dark:text-white">{detailLead.brands?.name}</div>
                  </div>
                  <div>
                    <span className="text-slate-400 font-medium">Score</span>
                    <div className="font-extrabold text-lg text-slate-900 dark:text-white">{detailLead.score}/100</div>
                  </div>
                  <div>
                ...
                <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/60 dark:border-slate-800">
                  <h3 className="text-xs font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                    <FileCode2 className="w-4 h-4 text-slate-500" /> Submitted Form Answers
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {detailLead.form_answers && Object.keys(detailLead.form_answers).length > 0 ? (
                      Object.entries(detailLead.form_answers).map(([key, val]) => (
                        <div key={key} className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800">
                ...
                <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/60 dark:border-slate-800">
                  ...
                  <div className="space-y-3">
                    {verifLogs.map((v) => (
                      <div key={v.id} className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 text-xs">
                        ...
                        <button
                          ...
                          className="text-slate-500 hover:text-slate-900 dark:hover:text-white text-[11px] font-semibold mt-1"
                        >
                ...
                <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/60 dark:border-slate-800">
                  ...
                  <div className="space-y-2 text-xs">
                    {deliveryLogs.length === 0 ? (
                      ...
                      deliveryLogs.map((d) => (
                        <div key={d.id} className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 flex items-center justify-between">
                          <div>
